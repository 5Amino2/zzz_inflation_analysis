...
本文**第2节**与**第3节**分别建立了危局强袭战与式舆防卫战的分数理论模型，**附录**（`zzz_inflation_analysis_appendix.md`）通过线性回归验证了两种挑战模式中怪物血量的实际膨胀规律。**本章的核心目的**：利用已验证的线性膨胀模型，帮助玩家**预测**自己未来的分数变化。
...
以下预测基于一个核心假设：怪物血量膨胀的线性趋势将持续。附录中的回归分析为这一假设提供了坚实的统计基础——危局排除异常后 $R^2$ 均值 0.9396，式舆个体中位数 $R^2 = 0.9811$。
...
| **个体回归验证** | 排除异常点后13种怪物全部 $R^2 \geq 0.85$，均值 $0.9396$（附录C.2） | 8种BOSS中5种 $R^2 \geq 0.95$，中位数 $0.9811$（附录C.4） |

> **注**：各期/节点平均血量的 $R^2$ 受怪物轮换影响而偏低，但附录中按怪物个体的回归证实了膨胀趋势具有高度统计显著性（危局排除2个异常点后100%个体 $R^2 \geq 0.85$，式舆62.5%个体 $R^2 \geq 0.95$）。
...
> **关键数据**：截至第41期，怪物拟合平均血量已膨胀至初始的 **4.08倍**（等效 $\alpha = 308.23\%$）。附录C.2进一步确认：排除2个线性异常点后，13种怪物的个体 $R^2$ 全部 $\geq 0.85$（均值 $0.9396$），膨胀具有高度稳定的线性特征。
...
> **关键数据**：截至节点13，BOSS拟合平均血量已膨胀至初始的 **1.51倍**（等效 $\alpha = 50.98\%$）。式舆的 $R^2 = 0.6354$ 受节点数少和BOSS轮换影响，但附录C.4按个体回归的中位数 $R^2 = 0.9811$，证实了膨胀的高度线性。每节点增长率约4.25%，而危局约7.71%/期，两模式长期膨胀强度可比。
...
已整理至配套附录文档 `zzz_inflation_analysis_appendix.md`，其结构如下：

| 附录章节 | 内容 |
|:---|:---|
| **A. 原始数据** | 第1\~41期共123条怪物血量记录、18种怪物的出现统计、式舆防卫战13节点39条BOSS数据 |
| **B. 线性回归结果** | 危局15种怪物 + 式舆8种BOSS的斜率、截距、R²、拟合函数表达式、统计汇总 |
| **C. 分数衰减预测数据** | 两种模式的等效膨胀系数表、A/B两类玩家的分数预测表、掉分节点/期数表、打满时间表 |
| **D. 代码与图表** | 项目文件结构说明、代码目录、图表输出目录（详见附录文档） |
...
| `src/theory_charts.py` | 理论模型图表（fig1\~fig8） |
