...
| $H$ | 怪物基准总血量 | 血量单位 |
| $\alpha$ | 血量膨胀系数 | 无量纲 |
| $T_{\text{full}}$ | 危局强袭战：无膨胀时打满60000分的时间 | 秒 |
| $T_{\text{base}}$ | 式舆防卫战：无膨胀时通关时间 | 秒 |
| $S$ | 最终得分 | 分 |

### 1.3 分析框架：四类玩家定义

本文的分析贯穿一个核心框架：将玩家在**两个维度**上进行划分，形成四种分析场景。

**维度一：挑战模式**

| 模式 | 限时 | 满分 | 奖励线 | 计分机制 |
|:---|:---:|:---:|:---:|:---|
| **危局强袭战** | 180秒 | 65000分 | 20000分 | 分段线性累计伤害映射（第2节） |
| **式舆防卫战** | 300秒 | 50000分 | 25000分 | 时间加权积分（第3节） |

**维度二：玩家目标**

| 玩家类型 | 定义 | 评判标准 | 核心关切 |
|:---|:---|:---|:---|
| **满分玩家** | 当前能拿满分的玩家，拥有**时间冗余** | 无膨胀时的通关时间 $T_{\text{full}}$（危局强袭战）或 $T_{\text{base}}$（式舆防卫战） | 膨胀后通关时间何时超过限时，即何时掉出满分线 |
| **奖励线玩家** | 不能满分，但分数高于奖励线的玩家，**时间冗余已耗尽** | 当前版本的分数 $S$ | 膨胀后分数何时跌破奖励线 |

**四类分析场景**

两个维度交叉，形成四种分析场景，对应本文各章节：

| 场景 | 模式 | 目标 | 理论分析 | 实证预测 |
|:---|:---|:---|:---|:---|
| 场景一 | 危局强袭战 | 满分 | §2.4.1 满分临界 | §6.2.3 A类玩家 |
| 场景二 | 危局强袭战 | 奖励线 | §2.4.2 奖励线临界 | §6.2.4 B类玩家 |
| 场景三 | 式舆防卫战 | 满分 | §3.4.1 满分临界 | §6.3.3 A类玩家 |
| 场景四 | 式舆防卫战 | 奖励线 | §3.4.2 奖励线临界 | §6.3.4 B类玩家 |

> **关键区别**：满分玩家的安全边际在**时间维度**（提前打完的比例），奖励线玩家的安全边际在**分数维度**（当前分数距奖励线的缓冲）。满分玩家中，$T_{\text{full}}$（或 $T_{\text{base}}$）越小则强度越高；奖励线玩家中，当前分数 $S$ 越高则强度越高。详见第6.1.1节的参数化方式。
...
与危局强袭战同理，式舆防卫战奖励线玩家的天然度量是原始分数 $S_0$。通过数值求解（先由 $S_0$ 反解 $T_{\text{base}}$，再求使分数降至25000分的 $\alpha$），得到：
...
**式舆防卫战的缓冲更"线性"**：与危局强袭战的超比例缓冲不同，式舆防卫战的积分模型使临界膨胀随分数大致线性增长。30000分→40000分（+10000分缓冲），临界膨胀从28.3%升至95.4%（增加约67个百分点）；40000分→50000分（同样+10000分缓冲），临界膨胀从95.4%升至270.3%（增加约175个百分点）。高分段仍有一定加速，但不如危局强袭战剧烈。
...
**直观理解**：
- 危局强袭战：你比180秒限时快了多少，就能承受多少膨胀
- 式舆防卫战：你比60秒满分边界快了多少，就能承受多少膨胀

![两模式归一化分数对比：膨胀对满分玩家的影响](charts/theory/fig3_normalized_comparison.png)

### 4.2 奖励线安全边际对比

**公平对比**应在归一化框架下进行，以两个模式的奖励线为目标，衡量当前的实力能够承受的膨胀率，危局强袭战能够承受更高的膨胀，——见下方图8。
...
- **刚好满分**（危局强袭战 $T_{\text{full}}=180\text{s}$，式舆防卫战 $T_{\text{base}}=60\text{s}$）：任何膨胀立即掉分
- **提前50%**（危局强袭战 $T_{\text{full}}=90\text{s}$，式舆防卫战 $T_{\text{base}}=30\text{s}$）：可承受100%膨胀
- **提前83%**（危局强袭战 $T_{\text{full}}=30\text{s}$，式舆防卫战 $T_{\text{base}}=10\text{s}$）：可承受500%膨胀
...
- **危局强袭战**：原始分数 $S_0=65000$（刚好满分）的玩家可承受355.7%膨胀才掉出20000分。这是因为20000分仅要求15000伤害分，对应约21.94%总血量，限时180秒内即使DPS不变，膨胀4.5倍后仍可在时间内达到。分数越低的玩家缓冲越小：$S_0=25000$（缓冲仅5000分）仅能承受28.1%膨胀。
- **式舆防卫战**：原始分数 $S_0=50000$（刚好满分）的玩家可承受270.3%膨胀才掉出25000分。$S_0=35000$（缓冲10000分）的玩家可承受57.8%膨胀。式舆防卫战的奖励线门槛占总分比例更高（50% vs 危局强袭战的30.8%），因此中低分玩家的缓冲相对更薄。

### 5.3 最脆弱的玩家群体

- **危局强袭战**：$S_0=20000$（刚好奖励线），任何膨胀立即掉出；$S_0=25000$（缓冲5000分），仅能承受28.1%膨胀
- **式舆防卫战**：$S_0=25000$（刚好奖励线），任何膨胀立即掉出；$S_0=30000$（缓冲5000分），仅能承受28.3%膨胀
- **式舆防卫战**：$S_0=28000$（缓冲3000分），仅17.0%膨胀就掉出25000分

### 5.4 设计启示

从时间参数化视角，两模式的满分安全边际完全对称，但奖励线安全边际差异显著：
- 危局强袭战的分段线性映射为低分目标提供了**超比例缓冲**
- 式舆防卫战的积分模型为低分目标提供了**线性缓冲**
- 式舆防卫战的阶梯倍率实际上为**中低强度玩家提供了更强的"膨胀免疫"**
...
以下预测基于一个核心假设：怪物血量膨胀的线性趋势将持续。下文6.1.3节的回归分析为这一假设提供了坚实的统计基础——危局强袭战排除异常后 $R^2$ 均值 0.9396，式舆防卫战个体中位数 $R^2 = 0.9811$。

#### 6.1.1 两类玩家定义（实证参数化）

第1.3节已定义了贯穿全文的四类玩家分析框架（危局强袭战/式舆防卫战 × 满分玩家/奖励线玩家）。本节对其进行**实证参数化**——为两类玩家赋予可量化的强度标记，以便在第6.2\~6.3节中用实际膨胀数据进行数值预测。
...
- **A类玩家**的冗余在**时间维度**：虽然都是满分（$S = S_{\max}$），但通关时间 $T$ 不同，$T$ 越小安全边际越大。**参数化方式**：用 $T_{\text{full}}$（危局强袭战）或 $T_{\text{base}}$（式舆防卫战）标记其强度。
...
> **注**：各期/节点平均血量的 $R^2$ 受怪物轮换影响而偏低，但按怪物个体的回归证实了膨胀趋势具有高度统计显著性（危局强袭战排除2个异常点后100%个体 $R^2 \geq 0.85$，式舆防卫战62.5%个体 $R^2 \geq 0.95$）。详见下文6.1.3节回归统计汇总。
...
**式舆防卫战：个体BOSS线性拟合汇总（排除R²<0.85异常点）**

与危局强袭战的分析流程相同，对式舆防卫战中R²<0.85的BOSS进行剔除：**秽息行者·蝎骸**（R²=0.7085）、**秽蚀·色雷斯人**（R²=0.7016）、**秽蚀·狛野真斗**（R²=0.5425），共3种。

| 统计指标 | 全部8种 | 排除异常点（5种） |
|----|:---|:---|
| 参与回归的BOSS数 | 8 种 | **5 种** |
| R² ≥ 0.95（极佳，Highly Linear） | 5 种（62.5%） | **5 种（100.0%）** |
| 0.70 ≤ R² < 0.85（一般，Moderate） | 2 种（25.0%） | 0 种（0%） |
| R² < 0.70（较差） | 1 种（12.5%） | 0 种（0%） |
| R² 均值 | 0.8610 | **0.9870** |
| R² 中位数 | 0.9811 | **0.9838** |
| 平均膨胀速率（斜率均值） | 134.99 万/节点 | 156.9 万/节点 |
| 膨胀速率中位数 | 107.3 万/节点 | 114.0 万/节点 |
| 所有BOSS R² ≥ 0.95 | 否（5/8） | **是（5/5，100%）** |

> **注**：排除3个R²<0.85的BOSS后，剩余5种BOSS的R²**全部≥0.95**（均值0.9870），说明式舆防卫战中核心BOSS的血量膨胀同样具有**高度稳定的线性特征**。对应的R² vs 膨胀速度散点图见 `charts/shiyu/chart_r2_vs_growth_filtered.png`。
...
| 怪物种类 | 18种 | 14种 |
| 不可回归/异常怪物 | 2种（R²<0.85） | 3种（R²<0.85） |
| R² 均值 | 0.8940（全部）/ **0.9396（排除异常）** | 0.8610（全部）/ **0.9870（排除异常）** |
| 高线性（R²≥0.95）占比 | 46.7%（全部）/ **53.8%（排除异常）** | 62.5%（全部）/ **100.0%（排除异常）** |
| 排除异常后全量R²≥0.85 | **是（13/13，100%）** | **是（5/5，100%）** |
| 每期/节点HP增幅 | \~359 万/期（中位数） | \~107.3 万/节点（中位数，全部）/ \~114.0 万/节点（排除异常） |
...
4. **式舆防卫战BOSS血量波动较大，但核心BOSS膨胀高度线性**：部分BOSS（如秽蚀·狛野真斗R²=0.5425）的R²较低，说明式舆防卫战中同一BOSS在不同节点的血量波动较大，可能存在非线性的数值调整策略（如特定节点的难度调整）。但排除3个R²<0.85的BOSS后，剩余5种核心BOSS的R²**全部≥0.95**（均值0.9870），膨胀的线性特征与危局强袭战同样高度稳定。

5. **两模式膨胀速率可比**：若将式舆防卫战的13个节点对应到约7个月的时间跨度，月均2个节点，则平均每节点增幅约107万HP，折合每月约214万HP。危局强袭战每期（约2周）约359万HP，折合每月约718万HP。危局强袭战的绝对膨胀速率约为式舆防卫战的3.4倍，这与危局强袭战作为高难挑战模式、怪物血量基数更大的定位一致。
...
4. 两模式的计分规则不同（危局强袭战为累计伤害分段计分，式舆防卫战为通关时间效率计分），HP膨胀对玩家分数的影响机制需要分别建模分析，详见本文第2节与第3节。
...
| $T_{\text{full}}$ | 第1期 | 第5期 | 第10期 | 第20期 | 第30期 | 第40期 | 第50期 | 第60期 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 30s | 65000 ★ | 65000 ★ | 65000 ★ | 65000 ★ | 65000 ★ | 65000 ★ | 65000 ★ | 65000 ★ |
| 45s | 65000 ★ | 65000 ★ | 65000 ★ | 65000 ★ | 65000 ★ | 64934 ○ | 57330 ○ | 51838 ○ |
| 60s | 65000 ★ | 65000 ★ | 65000 ★ | 65000 ★ | 61573 ○ | 53152 ○ | 47498 ○ | 43532 ○ |
| 75s | 65000 ★ | 65000 ★ | 65000 ★ | 63769 ○ | 52819 ○ | 46183 ○ | 41789 ○ | 37027 ○ |
| 90s | 65000 ★ | 65000 ★ | 65000 ★ | 56109 ○ | 47050 ○ | 41644 ○ | 35972 ○ | 31565 ○ |
| 120s | 65000 ★ | 65000 ★ | 59605 ○ | 46616 ○ | 39376 ○ | 32619 ○ | 27767 ○ | 24105 ○ |
| 150s | 65000 ★ | 61094 ○ | 51245 ○ | 41084 ○ | 32352 ○ | 26552 ○ | 22496 ○ | **19779** |
| 180s | 65000 ★ | 53879 ○ | 45787 ○ | 34992 ○ | 27353 ○ | 22363 ○ | **19281** | **17201** |
...
> **关键数据**：截至节点13，BOSS拟合平均血量已膨胀至初始的 **1.51倍**（等效 $\alpha = 50.98\%$）。式舆防卫战的 $R^2 = 0.6354$ 受节点数少和BOSS轮换影响，但6.1.3节按个体回归的中位数 $R^2 = 0.9811$，证实了膨胀的高度线性。每节点增长率约4.25%，而危局强袭战约7.71%/期，两模式长期膨胀强度可比。

#### 6.3.2 预测总览：不同强度玩家的分数衰减轨迹

下表展示9档典型强度（以 $T_{\text{base}}$ 标记）的玩家分数随节点变化的**预测值**：

| $T_{\text{base}}$ | 节点1 | 节点5 | 节点10 | 节点15 | 节点20 | 节点25 | 节点30 | 节点35 | 节点40 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 20s | 50000 ★ | 50000 ★ | 50000 ★ | 50000 ★ | 50000 ★ | 50000 ★ | 50000 ★ | 50000 ★ | 50000 ★ |
| 30s | 50000 ★ | 50000 ★ | 50000 ★ | 50000 ★ | 50000 ★ | 49918 ○ | 49114 ○ | 48111 ○ | 46983 ○ |
| 40s | 50000 ★ | 50000 ★ | 50000 ★ | 49494 ○ | 48315 ○ | 46758 ○ | 45036 ○ | 43190 ○ | 41540 ○ |
| 50s | 50000 ★ | 50000 ★ | 48875 ○ | 46977 ○ | 44820 ○ | 42560 ○ | 40416 ○ | 38391 ○ | 36420 ○ |
| 60s | 50000 ★ | 48740 ○ | 46287 ○ | 43624 ○ | 41071 ○ | 38603 ○ | 36237 ○ | 34027 ○ | 31961 ○ |
| 70s | 48781 ○ | 46511 ○ | 43397 ○ | 40408 ○ | 37556 ○ | 34907 ○ | 32460 ○ | 30277 ○ | 28442 ○ |
| 80s | 46933 ○ | 44076 ○ | 40620 ○ | 37354 ○ | 34366 ○ | 31631 ○ | 29319 ○ | 27409 ○ | 25804 ○ |
| 90s | 44904 ○ | 41756 ○ | 37955 ○ | 34540 ○ | 31470 ○ | 28931 ○ | 26876 ○ | 25178 ○ | **23751** |
| 100s | 42747 ○ | 39381 ○ | 35459 ○ | 31949 ○ | 29056 ○ | 26772 ○ | **24922** | **23432** | **22316** |
| 110s | 40739 ○ | 37149 ○ | 33149 ○ | 29711 ○ | 27081 ○ | 25004 ○ | **23371** | **22163** | **21226** |
| 120s | 38844 ○ | 35075 ○ | 30998 ○ | 27846 ○ | 25436 ○ | **23552** | **22212** | **21189** | **19773** |
...
- **$T_{\text{base}} = 30$s**（很强）：剩余12节点。已在节点10掉出满分——比危局强袭战中对应强度的玩家（$T_{\text{full}}=60$s，第27期掉出）更早失去满分。
- **$T_{\text{base}} = 40$s**（较强）：恰好在当前节点（节点13）处于边界（通关时间刚好60秒），下一节点将掉出。
- **$T_{\text{base}} \geq 45$s**：早已掉出满分线，构成B类玩家主体。

##### 与危局强袭战满分安全边际的理论对比

| 对应强度 | 危局强袭战满分临界 $\alpha$ | 式舆防卫战满分临界 $\alpha$ | 危局强袭战/式舆防卫战比 |
|:---|:---:|:---:|:---:|
...
> 式舆防卫战的满分安全边际仅为危局强袭战的 **1/2.5 到 1/5**。60秒满分边界的狭窄性使得除顶级竞速者外的所有玩家都无法在膨胀中维持满分。
...
- **当前得分 27000分的玩家**：仅剩**5节点**（约2.5个月）。这是式舆防卫战中最紧迫的群体。
- **与危局强袭战的差异**：式舆防卫战奖励线（25000）占总分50%，危局强袭战仅30.8%。式舆防卫战B类玩家的缓冲相对更薄——同等"接近奖励线"的玩家，式舆防卫战给的剩余时间更短。
...
核心预测：在现有膨胀趋势下，危局强袭战满分将在约12.5个月后成为仅极少数顶级竞速玩家（$T_{\text{full}} \leq 15$s）的专属；式舆防卫战满分将在约18个月后仅剩 $T_{\text{base}} \leq 10$s 的玩家。

#### 6.4.2 预测：奖励线韧性将持续分化

| 维度 | 危局强袭战（20000分线） | 式舆防卫战（25000分线） | 预测趋势 |
|:---|:---|:---|:---|
| **缓冲机制** | 分段线性 → 超比例缓冲 | 积分加权 → 线性缓冲 | 危局强袭战低分段缓冲消耗加速 |
| **奖励线占比** | 30.8%（门槛低） | 50.0%（门槛高） | 差距将维持 |
| **最危险B类** | 22000分 → 仅剩7期 | 27000分 → 仅剩5节点 | 两个群体均将在半年内掉出 |
| **安全B类** | ≥50000分 → ≥112期 | ≥45000分 → ≥53节点 | 安全边际充裕，但需关注趋势 |

核心预测：危局强袭战的超比例缓冲虽然在理论上更大，但低分玩家（≤25000分）的缓冲已经非常薄（临界膨胀<30%），在高膨胀速率下会首先出局。式舆防卫战则因为膨胀速率较慢（约4.25%/节点 vs 危局强袭战的7.71%/期），整体缓冲消耗更平缓。
...
|:---|:---|:---|:---|:---|
| **危局强袭战** | $T_{\text{full}} \leq 30$s | $T_{\text{full}} \in (30, 45]$s | $T_{\text{full}} > 45$s | 提前规划DPS提升路径 |
| **式舆防卫战** | $T_{\text{base}} \leq 20$s | $T_{\text{base}} \in (20, 40]$s | $T_{\text{base}} > 40$s | 关注60秒满分边界的逼近 |

**B类（奖励线玩家）**：

| 当前分数 | 危局强袭战剩余 | 式舆防卫战剩余 | 预测与建议 |
|:---|:---|:---|:---|
...
1. **膨胀稳定性支撑预测可信度**：危局强袭战排除2个异常点后100%个体 $R^2 \geq 0.85$（均值0.9396），式舆防卫战个体中位数 $R^2 = 0.9811$——两种模式的个体膨胀均具有高度线性特征，为本节的趋势预测提供了坚实的统计基础。

2. **新怪物冲击是最大不确定性**：新登场怪物的起始血量持续走高（跨版本"阶梯膨胀"），叠加个体线性膨胀，实际体验可能比本节的纯线性预测更为严峻。建议在新怪物登场后重新运行回归以更新预测。

3. **最紧迫的预测**：危局强袭战当前22000分玩家（7期后掉出奖励线，约3.5个月）和式舆防卫战当前27000分玩家（5节点后掉出奖励线，约2.5个月），如果不能在短期内通过装备或手法提升DPS 15\~20%，将很快失去奖励线保障。

#### 6.4.5 预测的局限性与更新频率

本节的预测基于截至第41期（危局强袭战）/第13节点（式舆防卫战）的线性回归参数，其准确性依赖于以下假设的持续成立：

1. **线性膨胀趋势持续**：当前验证了高度线性特征，但游戏运营可能随时调整数值策略。
2. **无结构性跳跃**：新怪物引入不产生跨越式的血量基线变化（附录数据提示存在此风险）。
3. **计分机制不变**：分段线性映射（危局强袭战）和时间加权积分（式舆防卫战）的规则保持不变。

**建议更新频率**：每5\~10期/节点用最新数据重新运行回归，刷新预测表格。由于两种模式数据的时间跨度不同（危局强袭战约19个月，式舆防卫战约7个月），跨模式预测比较时需考虑预测期的置信差异。

---

*本节数据基于截至第41期（危局强袭战）和第13节点（式舆防卫战）的回归参数，预测假设线性膨胀趋势持续。由于两种模式数据的时间跨度不同（危局强袭战约19个月，式舆防卫战约7个月），跨模式比较时需考虑预测期的置信差异。*
*更新时间：2026-06-04*
...
通过全文的理论建模与实证分析，我们得到三个相互关联的核心结论。

**结论一：伤害与得分之间存在非线性关系——这是理解膨胀影响的关键。**

- 危局强袭战的累计伤害与得分通过**29条血条的分段线性映射**关联，中间血条效率最高（960分/血量），外圈血条效率最低（520分/血量），差异接近2倍。这意味着同样的DPS损失，打在外圈血条上比内圈扣的分更狠。
- 式舆防卫战的得分是**伤害时间积分**，由阶梯倍率函数加权。前60秒5.0×，之后逐级下降，150秒后仅1.0×。首领战拖得越久，加权平均倍率越低，分数被一步步拖下来。
- 两种模式都存在**固定基础得分**（危局技术分+外圈低效率分数，式舆普通+精英固定10000分），这部分为所有玩家提供了"膨胀免疫"的底线保障。

这种非线性设计的**核心目的**：通过非线性的得分方式，同时实现安抚轻度玩家和激励深度玩家——让每个分数段玩家的抽卡意愿和满意度综合最大化。

**结论二：危局强袭战的膨胀环境比式舆防卫战（改版后）更平缓、更安全。**

| 对比维度 | 危局强袭战 | 式舆防卫战 |
|:---|:---|:---|
| 满分到奖励线的膨胀缓冲 | 355.7% | 270.3% |
| 满分安全边际（同等相对实力） | 临界膨胀的 2.5~5 倍 | 较低 |
| 个体膨胀线性度（排除异常） | R² 均值 0.9396 | R² 均值 0.9870（排除后） |
| 每期/节点相对涨幅 | ~7.71%/期 | ~4.25%/节点 |

危局强袭战满分到及格线（20000分）需要膨胀355.7%，式舆防卫战满分到及格线（25000分）仅需膨胀270.3%。尽管危局的绝对膨胀速率更高（约7.71%/期 vs 4.25%/节点），但其分段线性映射提供了**超比例缓冲**——分数越低，每分血量对应的分数"性价比"反而越高，使得低分玩家在膨胀面前比式舆防卫战的同等玩家更安全。

**结论三：同种怪物的膨胀呈现高度线性特征——这为我们的预测提供了统计基础。**

- 危局强袭战：排除2个异常点后，100%的个体怪物 R² ≥ 0.85（均值0.9396）
- 式舆防卫战：排除3个异常点后，100%的个体BOSS R² ≥ 0.95（均值0.9870）

为什么同种怪物的膨胀大多是线性的？因为游戏需要保持**可预期的难度增长曲线**——同种怪物具有完全相同的属性和AI，运营方会预设一个线性增长的基础血量模型，再根据当期角色强度调整环境buff和怪物血量。线性膨胀便于内部数值管理和玩家预期控制。

**综合设计洞察**：

两种模式都通过精巧的计分机制实现了"保护弱者、施压强者"的数值设计目标：
- 危局强袭战的分段线性映射为低分玩家提供了**超比例缓冲**（分数越低，每1%血量膨胀导致的分数损失越小）
- 式舆防卫战的阶梯倍率为中低强度玩家提供了**积分缓冲**（只要能在高倍率时段击杀精英和普通敌人，基础10000分几乎不受膨胀影响）
- 策划的核心思路是：让大部分玩家始终处于"再抽一张新卡就能保级"的心理安全区，同时让顶端玩家感受到竞争压力

### 7.2 局限性

本文分析的**局限性**：

- **膨胀用线性拟合的效果不佳**：很多怪物的膨胀并非按照线性，而且虽然单个怪物的膨胀可以用线性拟合，但是整体的血量和的线性拟合效果并不好，如果想要探究背后规律，可能可以选用其它拟合模型，例如指数模型
- **未来膨胀速度不可预测**：深渊的血量膨胀速度和角色强度的膨胀速度，最终取决于运营方的数值策略。本文基于历史数据的线性外推，但策划随时可能改变方向——这一点完全不可控。
- **天气（环境 Buff）的影响非常大**：每期/节点的天气加成可以在很大程度上左右最终得分表现（±10~20%甚至更多）。本文的模型假设恒定DPS，未纳入天气变量的影响，这将导致单期预测存在偏差。
...
基于第6节的预测分析，我们为不同处境的玩家提供分层建议：

**A类：满分/竞速玩家**

| 模式 | 可维持满分 | 即将掉出 | 已掉出 | 建议 |
|:---|:---|:---|:---|:---|
| **危局强袭战** | $T_{\text{full}} \leq 30$s（剩余25期） | $T_{\text{full}} \in (30, 45]$s | $T_{\text{full}} > 45$s | 提前规划DPS提升路径 |
| **式舆防卫战** | $T_{\text{base}} \leq 20$s（剩余36节点） | $T_{\text{base}} \in (20, 40]$s | $T_{\text{base}} > 40$s | 关注60秒满分边界的逼近 |

**B类：奖励线玩家**

| 当前分数 | 危局强袭战剩余 | 式舆防卫战剩余 | 建议 |
|:---|:---|:---|:---|
| 高分档（60000/49000） | 163期 | 81节点 | 无忧，正常养成，每期重跑预测刷新 |
| 中分档（45000/42000） | 86期 | 41节点 | 中长期安全，正常养成 |
| 低分档（30000/30000） | 30期 | 11节点 | 需评估DPS提升，约1~1.5年内有风险 |
| 边缘档（22000/27000） | **7期！** | **5节点！** | 紧急：3个月内需提升DPS 15~20% |

> **记住**：这只是游戏。数据和分析是为了帮你做出知情决策，而不是制造焦虑。你开心就好。

### 7.4 结语

> 大部分工作（数学建模、图表绘制、数据爬取、数据处理、PPT大纲生成、PPT生成等）由 AI 完成（当然核心想法和监督还是我自己做的）。感谢梁圣让大家用上了便宜的 AI。
>
...
| **B. 原始数据（式舆防卫战）** | 式舆防卫战13节点39条BOSS数据、BOSS透视表、节点与赛季对应 |
| **C. 初步分析结果** | 危局强袭战15种怪物 + 式舆防卫战8种BOSS的斜率、截距、R²等个体拟合统计（汇总已迁移至主文档6.1.3\~6.1.4节） |
...
| 文件 | 功能 |
|:---|:---|
| `src/weiju/analyze_inflation.py` | 危局强袭战怪物血量线性回归分析，生成拟合统计与图表 |
| `src/weiju/scrape_data.py` | 危局强袭战数据抓取 |
| `src/weiju/update_data.py` | 危局强袭战数据更新 |
| `src/shiyu/analyze_boss.py` | 式舆防卫战BOSS血量回归分析，生成统计与图表 |
| `src/shiyu/rebuild_analysis.py` | 式舆防卫战数据分析重建（含透视表和综合图表） |
| `src/shiyu/scrape_data.py` | 式舆防卫战数据抓取 |
